...
const SectionHome = () => {
    const [data, setData] = useState([])
      ...
      const filtradoNotebook = data.filter((notebook)=> notebook.categoria === "notebook")
      const filtradoMonitores = data.filter((monitor)=> monitor.categoria === "monitores")
      const filtradoFuentes = data.filter((fuente)=> fuente.categoria === "fuentes")
      const filtradoRegrigeracion = data.filter((refrigeracion)=> refrigeracion.categoria === "regrigeracion")
      const filtradoGabinetes = data.filter((gabinete)=> gabinete.categoria === "gabinetes")
 ...
 {data.length > 0 
  ?(
    ...
    {filtradoNotebook.map((product)=>(<TarjetaProductos key={product.id} product={product}/>)) }
    </div>
    </div>
    <div className='container-primerosProductos'>
    <h1 className='tituloTarjetaProductos'>Relacionado con Monitores</h1>
    <div className='tarjetaProductosFlex'>
    {filtradoMonitores.map((product)=>(<TarjetaProductos key={product.id} product={product}/>)) }
    </div>
    </div> 
    </>
  )
   ...
   <Link className={"link-tarjetaOfertas"} to={"/Detalle/nmRikxV4wgcF0sSCwZEj"}><TarjetaOfertas dcto={20} producto={"LG 27 27GL650F 144hz"} image={"https://res.cloudinary.com/dcf9eqqgt/image/upload/v1690400033/E-COMERCE%20CODER/Monitores/7/compragamer_Imganen_general_19484_Monitor_Gamer_LG_27__27GL650F_144hz_IPS_FHD_GSync_FreeSync_1e30b9f9-grn_pielws.jpg"} /></Link> 
    ...
    {data.length > 0 
      ?(
        ...
        <h1 className='tituloTarjetaProductos'>Relacionado con Notebooks</h1>
        <div className='tarjetaProductosFlex'>
        {filtradoFuentes.map((product)=>(<TarjetaProductos key={product.id} product={product}/>)) }
        </div>
        </div>
        <div className='container-primerosProductos'>
        <h1 className='tituloTarjetaProductos'>Relacionado con Monitores</h1>
        <div className='tarjetaProductosFlex'>
        {filtradoRegrigeracion.map((product)=>(<TarjetaProductos key={product.id} product={product}/>)) }
        </div>
        </div> 
        </>
      )
   ...
   <Link className={"link-tarjetaOfertas"} to={"/Detalle/j10scOBDxYZ5u6HVMxun"}><TarjetaOfertas dcto={20} producto={"GeForce RTX 3060"} image={"https://res.cloudinary.com/dcf9eqqgt/image/upload/v1690399812/E-COMERCE%20CODER/Placas%20de%20video/7/compragamer_Imganen_general_32951_Placa_de_Video_ASUS_GeForce_RTX_3070_Ti_8GB_GDDR6_ROG_STRIX_GAMING_OC_cbbbd3f6-grn_drzcyl.jpg"} /></Link> 
    ...
    {data.length > 0 
      ?(
        ...
        <h1 className='tituloTarjetaProductos'>Relacionado con Notebooks</h1>
        <div className='tarjetaProductosFlex'>
        {filtradoGabinetes.map((product)=>(<TarjetaProductos key={product.id} product={product}/>)) }
        </div>
        </div>
